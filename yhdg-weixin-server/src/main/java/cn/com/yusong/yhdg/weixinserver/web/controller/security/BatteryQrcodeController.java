...
@Controller
@RequestMapping(value = "/battery_qrcode")
public class BatteryQrcodeController extends AgentBaseController {
    ...
    public void index(String v, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Battery battery = batteryService.findByShellCode(v);
        if (battery == null) {
        ...
        if (appId > 0) {
            String url = String.format("%s/battery_qrcode/index_%d.htm?v=%s", appConfig.contextPath, appId, v);
    ...
    public void index(@PathVariable("appId") int appId, String v, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        v = "/battery_qrcode/index.htm?v=" + v;
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpIndex(appId, MP_PATH, "battery_qrcode", v, httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwIndex(appId, FW_PATH, "battery_qrcode", v, httpSession, response);
        }
    }
    ...
    public void systemSnsApiBase(@PathVariable("appId") int appId,  /*weixin*/String code,  /*alipay*/String auth_code, String state, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpSnsApiBase(appId, code, state, MP_PATH, "battery_qrcode", httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwSnsApiBase(appId, auth_code, state, FW_PATH, "battery_qrcode", httpSession, response);
        }
    }
