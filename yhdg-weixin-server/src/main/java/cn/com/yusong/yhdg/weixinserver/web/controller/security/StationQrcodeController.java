...
@Controller
@RequestMapping(value = "/station_qrcode")
public class StationQrcodeController extends AgentBaseController {

    private final static Logger log = LogManager.getLogger(StationQrcodeController.class);
    ...
    public void index(String v, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Station station = stationService.find(v);
        if (station == null) {
            responseText(response, "二维码无效");
            return;
        }
        if (station.getAgentId() == null) {
            responseText(response, "站点没有关联运营商");
            return;
        }
        ...
        if (appId > 0) {
            String url = String.format("%s/station_qrcode/index_%d.htm?v=%s", appConfig.contextPath, appId, v);
    ...
    public void index(@PathVariable("appId") int appId, String v, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        v = "/station_qrcode/index.htm?v=" + v;
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpIndex(appId, MP_PATH, "station_qrcode", v, httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwIndex(appId, FW_PATH, "station_qrcode", v, httpSession, response);
        }
    }
    ...
    public void systemSnsApiBase(@PathVariable("appId") int appId,  /*weixin*/String code,  /*alipay*/String auth_code, String state, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpSnsApiBase(appId, code, state, MP_PATH, "station_qrcode", httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwSnsApiBase(appId, auth_code, state, FW_PATH, "station_qrcode", httpSession, response);
        }
    }
