...
@Controller
@RequestMapping(value = "/vehicle_qrcode")
public class VehicleQrcodeController extends AgentBaseController {

    private final static Logger log = LogManager.getLogger(VehicleQrcodeController.class);
    ...
    public void index(String v, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        Vehicle vehicle = vehicleService.findByVinNo(v);
        if (vehicle == null) {
            responseText(response, "二维码无效");
            return;
        }
        if (vehicle.getAgentId() == null) {
            responseText(response, "车辆没有关联运营商");
            return;
        }
        ...
        if (appId > 0) {
            String url = String.format("%s/vehicle_qrcode/index_%d.htm?v=%s", appConfig.contextPath, appId, v);
    ...
    public void index(@PathVariable("appId") int appId, String v, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        v = "/vehicle_qrcode/index.htm?v=" + v;
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpIndex(appId, MP_PATH, "vehicle_qrcode", v, httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwIndex(appId, FW_PATH, "vehicle_qrcode", v, httpSession, response);
        }
    }
    ...
    public void systemSnsApiBase(@PathVariable("appId") int appId,  /*weixin*/String code,  /*alipay*/String auth_code, String state, HttpSession httpSession, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int browserType = getBrowserType(request);
        if (browserType == AppConstant.CLIENT_TYPE_MP) {
            systemMpSnsApiBase(appId, code, state, MP_PATH, "vehicle_qrcode", httpSession, response);
        } else if (browserType == AppConstant.CLIENT_TYPE_FW) {
            systemFwSnsApiBase(appId, auth_code, state, FW_PATH, "vehicle_qrcode", httpSession, response);
        }
    }
