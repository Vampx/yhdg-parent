...
public interface AlipayfwTemplateMessageMapper extends MasterMapper {
    ...
    public AlipayfwTemplateMessage find(int id);

    public int insert(AlipayfwTemplateMessage message);
     ...
     * @return 查询到的数据集合
     */
    List<AlipayfwTemplateMessage> findListByStatus(@Param("status") int status, @Param("limit") int limit, @Param("lastId") Long lastId);
    ...
    public int updateStatus(@Param("status") int status, @Param("id") long id, @Param("handleTime") Date handleTime);
