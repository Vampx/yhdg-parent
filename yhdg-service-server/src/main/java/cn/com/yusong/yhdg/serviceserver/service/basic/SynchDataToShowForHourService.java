...
@Service
public class SynchDataToShowForHourService extends AbstractService {
    static final Logger log = LogManager.getLogger(SynchDataToShowForHourService.class);
...
//        }
    ...
    private void synchBattery(){
        ...
        while (true) {
            ...
            agentIdList.add(9);
            List<Battery> batteryList = batteryMapper.findNotAgent(agentIdList, offset, limit);
    ...
    private void synchSubcabine(){
        ...
        while (true) {
            List<Integer> agentIdList = new ArrayList();
            agentIdList.add(9);
            List<Cabinet> cabinetList =  (List<Cabinet>) setAreaProperties(areaCache,cabinetMapper.findByNotAgent(agentIdList, offset, limit));
    ...
    private void synchSubcabineBox(){
        ...
        while (true) {
            List<Integer> agentIdList = new ArrayList();
            agentIdList.add(9);
            List<CabinetBox> cabinetBoxList = cabinetBoxMapper.findByNotAgent(agentIdList, offset, limit);
    ...
    private void synchCustomer(){
        ...
        while (true) {
            List<Integer> agentIdList = new ArrayList();
            agentIdList.add(9);
            List<Customer> customerList = customerMapper.findByNotAgent(agentIdList, offset, limit);
    ...
    private void synchFaultLog(){
        ...
        while (true) {
            ...
            List<Integer> agentIdList = new ArrayList();
            agentIdList.add(9);
            List<FaultLog> faultLogList = faultLogMapper.findByCursorNotAgent(agentIdList, Integer.parseInt(cursorId), limit);
    ...
    private void synchDataStats(){
        ...
        List<Integer> agentIdList = new ArrayList();
        agentIdList.add(9);
        int totalOrderCount = batteryOrderMapper.findTotalCountByNotAgent(agentIdList, null, todayBeginTime);
        ...
        for (String table : list) {
            ...
            totalOrderCount += batteryOrderHistoryMapper.findTotalCountByNotAgent(agentIdList, table, null, todayBeginTime);
            //log.debug("查询到历史表总换电数：{} ",totalOrderCount);
        }
        //log.debug("查询总换电数结束");

        //总异常换电数
        int faultLogCount = faultLogMapper.findTotalCountByNotAgent(agentIdList, null, todayBeginTime);
        //log.debug("总异常换电数：{} ",faultLogCount);

        //总换电地区排行
        //log.debug("查询总换电地区排行开始");
        List <BatteryOrder>  orderList = new ArrayList();
        orderList.addAll(batteryOrderMapper.findCountByCityAndNotAgent(agentIdList, null, todayBeginTime));
        //log.debug("总yhdg表排行size：{} ",orderList.size());
        for (String table : list) {
            //log.debug("查询到table：{} ",table);
            orderList.addAll(batteryOrderHistoryMapper.findCountByCityAndNotAgent(agentIdList, table));
