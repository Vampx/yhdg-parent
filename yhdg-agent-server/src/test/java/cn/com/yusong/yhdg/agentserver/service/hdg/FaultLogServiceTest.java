...
public class FaultLogServiceTest extends BaseJunit4Test {
    ...
    public void updateStatus() {
        ...
        BatteryOrder batteryOrder = newBatteryOrder(newOrderId(OrderId.OrderIdType.BATTERY_ORDER), systemBatteryType
                .getId(), partner.getId(), agent.getId(), battery.getId(), customer.getId());
        ...
        Long[] idList = new Long[]{faultLog1.getId(), faultLog5.getId(), faultLog2.getId(), faultLog3.getId(),
                faultLog4.getId(), faultLog.getId()};

        service.updateStatus(idList, "张三");
        assertNull(jdbcTemplate.queryForMap("select temp_fault_log_id from hdg_cabinet where id = ?", cabinet.getId()
        ).get("temp_fault_log_id"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog5.getId()).get("status"));

        assertNull(jdbcTemplate.queryForMap("select offline_fault_log_id from hdg_cabinet where id = ?", cabinet
                .getId()).get("offline_fault_log_id"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog1.getId()).get("status"));

        assertNull(jdbcTemplate.queryForMap("select all_full_fault_log_id from hdg_cabinet where id = ?", cabinet
                .getId()).get("all_full_fault_log_id"));
        assertEquals(0, jdbcTemplate.queryForMap("select all_full_count from hdg_cabinet where id = ?", cabinet.getId
                ()).get("all_full_count"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog3.getId()).get("status"));

        assertNull(jdbcTemplate.queryForMap("select pay_timeout_fault_log_id from hdg_battery_order where id = ?",
                batteryOrder.getId()).get("pay_timeout_fault_log_id"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog4.getId()).get("status"));

        assertNull(jdbcTemplate.queryForMap("select not_take_timeout_fault_log_id from hdg_battery_order where id = " +
                "?", batteryOrder.getId()).get("not_take_timeout_fault_log_id"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog2.getId()).get("status"));

        assertNull(jdbcTemplate.queryForMap("select not_electrify_fault_log_id from hdg_battery where id = ?",
                battery.getId()).get("not_electrify_fault_log_id"));
        assertEquals(FaultLog.Status.PROCESSED.getValue(), jdbcTemplate.queryForMap("select status from hdg_fault_log" +
                " where id = ?", faultLog.getId()).get("status"));
