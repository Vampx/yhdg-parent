...
import cn.com.yusong.yhdg.agentserver.persistence.hdg.InsuranceMapper;
import cn.com.yusong.yhdg.agentserver.persistence.hdg.CabinetBatteryTypeMapper;
import cn.com.yusong.yhdg.agentserver.persistence.hdg.ExchangeBatteryForegiftMapper;
import cn.com.yusong.yhdg.agentserver.persistence.hdg.PacketPeriodPriceMapper;
...
public class AgentBatteryTypeService extends AbstractService {
	...
	public ExtResult create(AgentBatteryType search) {
		...
		if (agentBatteryTypeMapper.insert(search) >= 1) {
	...
	public ExtResult update(AgentBatteryType search) {
		...
		if (search.getToBatteryType().intValue() != search.getBatteryType().intValue()) {
			ExchangeBatteryForegift exchangeBatteryForegift = exchangeBatteryForegiftMapper.findByBatteryType(search.getBatteryType(), search.getAgentId());
		...
		if (agentBatteryTypeMapper.update(search.getToBatteryType(), search.getTypeName(), search.getBatteryType(), search.getAgentId()) >= 1) {
