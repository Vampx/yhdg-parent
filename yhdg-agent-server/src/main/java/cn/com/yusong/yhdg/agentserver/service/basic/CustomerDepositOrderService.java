...
public class CustomerDepositOrderService  extends AbstractService {
    ...
    public int sumMonthMoney(int status, Date queryBeginDate, Date queryEndDate) {
        return customerDepositOrderMapper.sumMonthMoney(status, queryBeginDate, queryEndDate);
    }
...
//            try {
//
//                File certFile = AppUtils.getWeixinCertFile(appConfig.appDir, partner.getId());
//                if(!certFile.exists()) {
...
//            }
//
//            String appId,mchId,partnerKey;
////            if(weixinmpPayOrder.getPayAppId() == Constant.SYSTEM_PARTNER_ID){
////                appId = systemConfigMapper.findConfigValue(ConstEnum.SystemConfigKey.WXMP_APP_ID.getValue());
////                mchId = systemConfigMapper.findConfigValue(ConstEnum.SystemConfigKey.WXMP_PARTNER_ID.getValue());
////                partnerKey = systemConfigMapper.findConfigValue(ConstEnum.SystemConfigKey.WXMP_PARTNER_KEY.getValue());
////            } else {
////                appId = agentSystemConfigMapper.find(ConstEnum.SystemConfigKey.WXMP_APP_ID.getValue(), weixinmpPayOrder.getPayAppId()).getConfigValue();
////                mchId = agentSystemConfigMapper.find(ConstEnum.SystemConfigKey.WXMP_PARTNER_ID.getValue(), weixinmpPayOrder.getPayAppId()).getConfigValue();
////                partnerKey = agentSystemConfigMapper.find(ConstEnum.SystemConfigKey.WXMP_PARTNER_KEY.getValue(), weixinmpPayOrder.getPayAppId()).getConfigValue();
////            }
//
...
//                }
//
////                WxMpPrepayIdResult result = WxPayUtils.refundOrder(appId, mchId, partnerKey, outTradeNo, totalFee, refundFee, "客户充值退款", file);
////                if (result != null && result.getReturn_code().equals("SUCCESS")) {
...
//            String alipayfwPublic = partner.getAlipayAliKey();
//            String alipayfwAppRsaPrivate = partner.getAlipayPriKey();
//
//            double refundAmount = ((double) money) / 100;
