...
public class WithdrawService extends AbstractService {
    ...
    public ExtResult audit(String id, int fromStatus, int status, String auditUser, String auditMemo) {
        Withdraw withdraw = withdrawMapper.find(id);
        if (withdraw == null) {
        ...
        if (status == Withdraw.Status.AUDIT_OK.getValue()) {
            withdrawMapper.audit(id, fromStatus, status, new Date(), auditUser, auditMemo);

        } else if (status == Withdraw.Status.AUDIT_NO.getValue()) {
            if (withdrawMapper.audit(id, fromStatus, status, new Date(), auditUser, auditMemo) > 0) {
                if (withdraw.getType() == Withdraw.Type.CUSTOMER.getValue()) {
