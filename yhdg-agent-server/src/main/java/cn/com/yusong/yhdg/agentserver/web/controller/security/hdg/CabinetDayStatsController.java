...
import cn.com.yusong.yhdg.agentserver.service.hdg.CabinetDayStatsService;
...
@Controller
@RequestMapping(value = "/security/hdg/cabinet_day_stats")
public class CabinetDayStatsController extends SecurityController {
    ...
    static Logger log = LoggerFactory.getLogger(CabinetDayStatsController.class);

    @SecurityControl(limits = "hdg.CabinetDayStats:list")
    @RequestMapping(value = "index.htm")
    public void index(Model model) {
        List<Agent> agentList = agentService.findAll();
        model.addAttribute("agentList", agentList);
        model.addAttribute(MENU_CODE_NAME, "hdg.CabinetDayStats:list");
    }

    @RequestMapping("page.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public PageResult page(CabinetDayStats search) {
        return PageResult.successResult(cabinetDayStatsService.findPage(search));
    }

    @RequestMapping("export_excel.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody//到出至excel
    public ExtResult exportExcel(CabinetDayStats search, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            WritableSheet sheet = wwb.createSheet("换电柜日收入统计", 0);
            sheet.getSettings().setDefaultColumnWidth(20);
            sheet.setRowView(0, 400, false);
            /**
             * 设置题头
             */
            sheet.addCell(new Label(0, 0,"换电柜日收入统计", getTitleStyle()));
            int column = 0;
            ...
            while (true) {
                ...
                List<CabinetDayStats> list = cabinetDayStatsService.findForExcel(search);
                if (list.isEmpty()) {
                    break;
                }
                ExportXlsUtils.writeCabinetDayStates(offset, list, sheet);
                offset += limit;
            }
            // 写入数据
            wwb.write();
