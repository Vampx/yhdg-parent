...
public class CabinetService extends AbstractService {
    ...
    public int findCabinetCount(Integer agentId) {
        return cabinetMapper.findCabinetCount(agentId);
    }
    ...
    public ExtResult delete(String id) {
        ...
        if (cabinetCurrentFaultMapper.findCountByCabinet(id) > 0) {
            return ExtResult.failResult("存在当前故障信息不能删除！");
        }
    ...
    @Transactional(rollbackFor = Throwable.class)
    public ExtResult updateBasic(Cabinet cabinet) {
...
//        }
    ...
    public ExtResult updateRatio(String id, Integer platformRatio, Integer agentRatio, Integer provinceAgentRatio, Integer cityAgentRatio, Integer shopRatio) {
        int total = cabinetMapper.updateRatio(id, platformRatio, agentRatio, provinceAgentRatio, cityAgentRatio, shopRatio);
        if (total == 0) {
    ...
    public ExtResult unbindBatteryType(String cabinetId, Integer batteryType) {
        ...
        if (cabinet != null && cabinet.getShopId() != null) {
            ...
            if (vipPriceShop != null) {
                updateShopPrice(vipPriceShop.getPriceId(), vipPriceShop.getShopId());
            }else {
