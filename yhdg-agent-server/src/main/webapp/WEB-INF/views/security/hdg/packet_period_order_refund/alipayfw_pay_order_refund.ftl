...
<script>
    ...
    (function () {
        $('#page_table_box_${pid}').datagrid({
            ...
            width: '100%',
            height: '100%',
            url: "${contextPath}/security/hdg/packet_period_order_refund/alipayfw_pay_order_page_refund.htm?orderRefundId=${orderRefundId}",
            fitColumns: true,
            idField: 'id',
            singleSelect: true,
            selectOnCheck: false,
    ...
    function view_${pid}(customerId, sourceType, sourceId, orderStatus) {
        App.dialog.show({
            ...
            title: '查看',
            href: "${contextPath}/security/basic/alipayfw_pay_order/view.htm?customerId=" + customerId + "&sourceType=" + sourceType + "&sourceId=" +sourceId + "&orderStatus=" + orderStatus
