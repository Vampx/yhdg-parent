            ...
            <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td align="right" colspan="2">
                        <div style="width:580px; height:440px; overflow: hidden;margin-left: 10px; /*border:1px solid #ccc;*/">
                            <iframe id="map_frame_${pid}" src="${contextPath}/security/basic/baidu_map/cabinet_location.htm?lng=${(entity.lng)!''}&lat=${(entity.lat)!''}&address=${(entity.address)!''}" width="100%" height="100%" frameborder="0" style="border:0;"></iframe>
                        </div>
                    </td>
                </tr>
                <tr><td><div id="current_location" style="margin-left: 10px;">当前位置：<span id="current_position">${(entity.address)!''}</span><br>当前经度/纬度：<span id="current_lng_lat">${(entity.lng)!''}/${(entity.lat)!''}</span> </div></td><td align="right"><a href="javascript:void(0)" class="a_red map_location">修改为当前地址</a>&nbsp;&nbsp;&nbsp;</td></tr>
            </table>
        </form>
    </div>
</div>
            ...
            $.messager.confirm('提示信息', '确认修改为当前地址?', function(ok) {
                if (ok) {
                    ...
                    $.ajax({
                        ...
                        async: false,
                        type: 'POST',
                        url: '${contextPath}/security/hdg/cabinet/update_new_location.htm',
                        dataType: 'json',
                        data: values,
