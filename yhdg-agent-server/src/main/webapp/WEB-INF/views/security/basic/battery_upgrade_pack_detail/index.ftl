...
<script>
    (function() {
        ...
        var upgradePackId = windowData.batteryUpgradePackId;
        ...
        datagrid.datagrid({
            ...
            striped: true,
            pagination: true,
            url: "${contextPath}/security/basic/battery_upgrade_pack_detail/page.htm?upgradePackId="+upgradePackId,
            fitColumns: true,
            pageSize: 10,
            pageList: [10, 50, 100],
            idField: 'id',
        ...
        function add() {
            App.dialog.show({
                ...
                title: '增加升级电池',
                href: "${contextPath}/security/basic/battery_upgrade_pack_detail/add.htm",
                windowData: {
                    ok: function (checked) {
                        console.log(checked);
                        var batteryId = [];
                        for(var i=0; i<checked.length; i++){
                            batteryId.push(checked[i].id)
                        }
                        $.post('${contextPath}/security/basic/battery_upgrade_pack_detail/create.htm',
                                {
                                    batteryId:batteryId,
    ...
    function del(batteryId) {
        var win = $('#${pid}'), windowData = win.data('windowData');
        var upgradePackId = windowData.batteryUpgradePackId;
        $.messager.confirm('提示信息', '确认删除?', function (ok) {
            if (ok) {
                $.post('${contextPath}/security/basic/battery_upgrade_pack_detail/delete.htm?upgradePackId='+upgradePackId,
                        {
                            batteryId: batteryId
                        }, function (json) {
                            if (json.success) {
