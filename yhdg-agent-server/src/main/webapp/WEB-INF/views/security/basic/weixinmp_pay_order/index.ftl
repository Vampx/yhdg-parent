    ...
    <script>
        $(function() {
            $('#page_table').datagrid({
                ...
                striped: true,
                pagination: true,
                url: "${contextPath}/security/basic/weixinmp_pay_order/page.htm?<#if Session['SESSION_KEY_USER'].isSelfPlatform?? && Session['SESSION_KEY_USER'].isSelfPlatform == 1>appId<#else>agentId</#if>=${Session['SESSION_KEY_USER'].agentId}",
                fitColumns: true,
                pageSize: 10,
                pageList: [10, 50, 100],
                idField: 'id',
