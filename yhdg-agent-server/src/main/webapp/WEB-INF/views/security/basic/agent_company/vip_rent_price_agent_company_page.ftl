<div class="popup_body">
    <div class="select_routes" >
        <div class="select_box" style="width: 200px; height:402px; float:right;">
            ...
            <div class="select_body" style="top:59px; overflow: hidden;">
                ...
                <#if agentCompanyList ??>
                    <#list agentCompanyList as agentCompany>
                        <li agentCompanyId="${agentCompany.id}" agentCompanyName ="${(agentCompany.companyName)!''}"><span class="text" >${(agentCompany.companyName)!''}</span></li>
                    </#list>
                </#if>
                </ul>
            </div>
    ...
    <div class="search" style="margin:0 0; padding:2px 0 0 0; border-bottom:none;">
        <table cellspacing="0" cellpadding="0" border="0">
            <tr>
                <td align="right">运营公司编号：</td>
                <td><input type="text" style="width: 100px" class="text" id="agent_company_id_${pid}"/>&nbsp;&nbsp;</td>
                <td align="right">运营公司名称：</td>
                <td><input type="text" style="width: 100px" class="text"  id="agent_company_name_${pid}"/></td>
...
<script>
    ...
    (function() {
        ...
        datagrid.datagrid({
            ...
            striped: true,
            pagination: true,
            url: "${contextPath}/security/basic/agent_company/page.htm?agentId=${(agentId)!''}&vipRentFlag=1",
            fitColumns: true,
            pageSize: 10,
            pageList: [10, 50, 100],
            idField: 'id',
            ...
            columns: [
                [
                    ...
                    {title: '运营公司名称', align: 'center', field: 'companyName', width: 60},
                    {
        ...
        var line =
                '<li agentCompanyId="AGENT_COMPANY_ID" agentCompanyName="AGENT_COMPANY_NAME">' +
                '<span class="text">AGENT_COMPANY_ID/AGENT_COMPANY_NAME</span>' +
                '<p class="icon_btn"><a class="delete" title="删除" href="javascript:void(0)" onclick="deleteRow(this)"><span class="icon"></span></a>' +
                '</li>';


        var add = function(agentCompany) {
            if($('#selected_container_${pid} li[agentCompanyId="' + agentCompany.id +'"]').length > 0) {
                alert("运营公司已存在");
                return;
            }
            var name;
            if (agentCompany.companyName != null) {
                name = agentCompany.companyName;
            } else {
                name = "";
            }
            var html = line.replace(/AGENT_COMPANY_ID/g, agentCompany.id).replace(/AGENT_COMPANY_NAME/g, name);
        ...
        var getSelected = function() {
            ...
            selectedContainer.find('li').each(function() {
                var me = $(this);
                list.push({
                    agentCompanyId: me.attr('agentCompanyId')
                });
            });
            return list;
        };
        ...
        var query = function() {
            datagrid.datagrid('options').queryParams = {
                id: $('#shop_id_${pid}').val(),
                agentCompanyName: $('#agent_company_name_${pid}').val()
            };
