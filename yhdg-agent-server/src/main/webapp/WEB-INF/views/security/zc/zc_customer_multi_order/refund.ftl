<div class="popup_body" style="min-height: 82%;">
    <div class="ui_table">
        <form method="post">
            ...
            <input type="hidden" name="sourceType" <#if entity.sourceType ?? >value="${entity.sourceType}" </#if>>
            ...
            $.messager.confirm('提示信息', $('#total_money').html()+'<br/><br/>是否确认?', function (ok) {
                if (ok) {
                    ...
                    if(refundMoney == 0) {
                        ...
                        $.post('${contextPath}/security/zc/zc_customer_multi_order/refund_money.htm', {
                            data : $.toJSON(values)
                        }, function(json) {
                        <@app.json_jump/>
                            if(json.success) {
