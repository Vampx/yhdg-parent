    ...
    $(function() {
        $(".vip_customer_add").click(function () {
            App.dialog.show({
                ...
                title: '新建',
                href: "${contextPath}/security/zd/vip_rent_price_customer/add.htm?priceId="+ ${(priceId)!0} + "&agentId=" +${(agentId)!0},
                windowData: {
                },
                event: {
                    onClose: function() {
        ...
        $(".vip_price .fa-close").click(function () {
            ...
            $.messager.confirm('提示信息', '确认删除该骑手?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/zd/vip_rent_price_customer/delete.htm?id=" + id + "&priceId=" + priceId, function (json) {
                        if (json.success) {
