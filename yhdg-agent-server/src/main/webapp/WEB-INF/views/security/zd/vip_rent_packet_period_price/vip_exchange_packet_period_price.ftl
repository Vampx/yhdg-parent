    ...
    $(function() {
        $(".exchange_rent_add").click(function () {
            ...
            App.dialog.show({
                ...
                title: '添加租金',
                href: "${contextPath}/security/zd/vip_rent_packet_period_price/vip_exchange_packet_period_price_add.htm?batteryType="+ ${(batteryType)!''}+ "&agentId="+ ${(agentId)!''} + "&foregiftId=" + foregiftId + "&priceId=" + ${(priceId)!0} ,
                windowData:{
                },
                event: {
                    onClose: function() {
                        $.post('${contextPath}/security/zd/vip_rent_packet_period_price/vip_exchange_packet_period_price.htm', {
        ...
        $(".exchange_rent .fa-edit").click(function () {
            ...
            App.dialog.show({
                css: 'width:500px;height:470px;overflow:visible;',
                title: '修改租金',
                href: "${contextPath}/security/zd/vip_rent_packet_period_price/vip_exchange_packet_period_price_edit.htm?exchangeId=" + exchangeId,
                windowData:{
                },
                event: {
                    onClose: function() {
        ...
        $(".exchange_rent .fa-close").click(function () {
            ...
            $.messager.confirm('提示信息', '确认删除电池租金?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/zd/vip_rent_packet_period_price/vip_exchange_packet_period_price_delete.htm?exchangeId=" + exchangeId, function (json) {
                        if (json.success) {
