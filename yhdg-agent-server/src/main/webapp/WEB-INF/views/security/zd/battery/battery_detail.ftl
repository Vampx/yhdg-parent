...
//        //加载运动轨迹列表
    ...
    $('#switch').click(function () {
        ...
        App.dialog.show({
            ...
            title: '选择电池',
            href: "${contextPath}/security/zd/battery/switch_battery.htm",
            windowData: {
                ok: function(config) {
                    //切换电池
                    window.location.href = "${contextPath}/security/zd/battery/battery_detail.htm?id=" + config.battery.id;
                }
            },
            event: {
                onClose: function() {
...
//    })
    ...
    function battery_report_view(batteryId, createTime) {
        App.dialog.show({
            css: 'width:886px;height:647px;overflow:scroll;',
            title: '查看',
            href: "${contextPath}/security/hdg/battery_report/view_normal_heart.htm?batteryId=" + batteryId +"&createTime=" + createTime
        });
    }
