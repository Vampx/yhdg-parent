    ...
    <script>
        $(function() {
            $('#page_table').datagrid({
                ...
                striped: true,
                pagination: true,
                url: "${contextPath}/security/zd/rent_order/page.htm?agentId=${Session['SESSION_KEY_USER'].agentId}",
                fitColumns: true,
                pageSize: 50,
                pageList: [50, 100],
                idField: 'id',
        ...
        function view(id) {
            App.dialog.show({
                ...
                title: '查看',
                href: "${contextPath}/security/zd/rent_order/view.htm?id=" + id
        ...
        function updateStatus() {
            ...
            $.messager.confirm('提示信息', '是否结束订单?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/zd/rent_order/updateStatus.htm?id=" + list[0].id, function (json) {
                        if (json.success) {
        ...
        function exchangeBattery() {
            ...
            $.messager.prompt('提示信息', '请输入电池编号：', function (batteryId) {
                ...
                if (batteryId) {
                    $.post("${contextPath}/security/zd/rent_order/exchange_battery.htm?", {
                        id: list[0].id,
                        batteryId: batteryId
                    }, function (json) {
                        if (json.success) {
