...
import cn.com.yusong.yhdg.webserver.persistence.hdg.CabinetReportDateMapper;
...
@Service
public class CabinetReportDateService {
    @Autowired
    CabinetReportDateMapper cabinetReportDateMapper;

    public void tree(OutputStream stream, String cabinetId) throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        JsonGenerator json = objectMapper.getJsonFactory().createJsonGenerator(stream, JsonEncoding.UTF8);
        json.writeStartArray();
        YhdgUtils.writeJson(buildTree(cabinetId), json);
        json.writeEndArray();
    ...
    private List<Node<NodeModel>> buildTree(String cabinetId) {
        List<String> yearList = cabinetReportDateMapper.findYeah(cabinetId);
        List<Map<String, String>> monthMap = cabinetReportDateMapper.findMonth(cabinetId);
        List<CabinetReportDate> list = cabinetReportDateMapper.findList(cabinetId);
        ...
        for (String year : yearList) {
            ...
            for (Map<String, String> map : monthMap) {
                ...
                if (map.get("yeah").equals(year)) {
                    ...
                    for (CabinetReportDate date : list) {
