...
import cn.com.yusong.yhdg.webserver.service.basic.AgentCompanyTotalStatsService;
...
@Controller
@RequestMapping(value = "/security/basic/agent_company_total_stats")
public class AgentCompanyTotalStatsController extends SecurityController {
    ...
    static Logger log = LoggerFactory.getLogger(AgentCompanyTotalStatsController.class);

    @SecurityControl(limits = "basic.AgentCompanyTotalStats:list")
    ...
    public void index(Model model) {
        ...
        model.addAttribute(MENU_CODE_NAME, "basic.AgentCompanyTotalStats:list");
    }

    @RequestMapping("page.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public PageResult page(AgentCompanyTotalStats search) {
        if (search.getCategory() == null) {
            search.setCategory(ConstEnum.Category.EXCHANGE.getValue());
        }
        return PageResult.successResult(agentCompanyTotalStatsService.findPage(search));
    }

    @RequestMapping("export_excel.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody//到出至excel
    public ExtResult exportExcel(AgentCompanyTotalStats search, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            WritableSheet sheet = wwb.createSheet("运营公司总收入统计", 0);
            sheet.getSettings().setDefaultColumnWidth(20);
            sheet.setRowView(0, 400, false);
            /**
             * 设置题头
             */
            sheet.addCell(new Label(0, 0,"运营公司总收入统计", getTitleStyle()));
            int column = 0;
            ...
            while (true) {
                ...
                List<AgentCompanyTotalStats> list = agentCompanyTotalStatsService.findForExcel(search);
                if (list.isEmpty()) {
                    break;
                }
                ExportXlsUtils.writeAgentCompanyTotalStates(offset, list, sheet);
                offset += limit;
            }
            // 写入数据
            wwb.write();
