...
import cn.com.yusong.yhdg.webserver.service.basic.WithdrawService;
...
public class WithdrawController extends SecurityController {
    ...
    @SecurityControl(limits = "basic.WithdrawAgent:list")
    @RequestMapping("agent_index.htm")
    public void agentIndex(Model model) {
        model.addAttribute(MENU_CODE_NAME, "basic.WithdrawAgent:list");
        model.addAttribute("StatusEnum", Withdraw.Status.values());
        model.addAttribute("AccountTypeEnum", Withdraw.AccountType.values());
    }
    ...
    public PageResult page(Withdraw search) {
    ...
    public ExtResult offline(String id) {
        return withdrawService.offline(id);
    }
}