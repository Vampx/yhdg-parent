...
public class CabinetController extends SecurityController {
    ...
    @ViewModel(ViewModel.INNER_PAGE)
    @RequestMapping(value = "price_cabinet_page.htm")
    public void priceCabinetPage(Model model, Integer agentId) {
        model.addAttribute("agentId", agentId);
    }
    ...
    public String cabinetDetail(Model model, String id, HttpServletRequest request) {
        ...
        for (CabinetBox cabinetBox : cabinetBoxList) {
            if (StringUtils.isNotEmpty(cabinetBox.getBoxStateName()) && (cabinetBox.getBoxStateName().contains("报警") || cabinetBox.getBoxStateName().contains("异常"))) {
                cabinetBox.setFaultFlag(ConstEnum.Flag.TRUE.getValue());
            }
        }
    ...
    public String editShop(Model model, String cabinetId, Integer agentId) {
    ...
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public ExtResult updateBasic(Cabinet entity, HttpSession session) {
        long userId = getSessionUser(session).getId();
        ExtResult result = cabinetService.updateBasic(entity, userId);
    ...
    public ExtResult updateRatio(String id, Integer platformRatio, Integer agentRatio, Integer provinceAgentRatio, Integer cityAgentRatio, Integer shopRatio, Integer shopFixedMoney) {
        ExtResult result = cabinetService.updateRatio(id, platformRatio, agentRatio, provinceAgentRatio, cityAgentRatio, shopRatio, shopFixedMoney);
        return result;
    }
