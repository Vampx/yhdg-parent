...
public class AgentAgentController extends SecurityController {
    ...
    @SecurityControl(limits = "basic.AgentAgent:list")
    ...
    public void index(Model model, HttpSession httpSession, Integer parentId) {
        ...
        model.addAttribute(MENU_CODE_NAME, "basic.AgentAgent:list");
    }

    @RequestMapping("page_tree.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public PageResult pageTree(Agent search) {
    ...
    public String setPayPeople(Model model, String id) {
        ...
        return "/security/basic/agent_agent/set_pay_people";
    }
    ...
    public ExtResult setPayPeopleUpdate(Agent entity) {
        return  agentService.setPayPeopleUpdate(entity);
    }

    @RequestMapping(value = "shop_clearing.htm")
    public void shopClose(Model model) {
    }

    @RequestMapping("clearing.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public ExtResult clearing(String[] ids, Integer agentId, String shopId, Date beginTime, Date endTime, Integer balance, HttpSession httpSession) {
        SessionUser sessionUser = getSessionUser(httpSession);
        String userName = sessionUser.getUsername();
        return agentService.clearing(agentId, shopId, balance, userName, ids, beginTime, endTime);
    }

    @ViewModel(ViewModel.INNER_PAGE)
    @RequestMapping(value = "view_agent_shop_balance_record.htm")
    public String viewAgentShopBalanceRecord(Model model, int id) {
        Agent entity = agentService.find(id);
        if (entity == null) {
            return SEGMENT_RECORD_NOT_FOUND;
        } else {
        ...
        return "/security/basic/agent_agent/view_agent_shop_balance_record";
    }
}
