...
import cn.com.yusong.yhdg.webserver.service.hdg.AgentTotalStatsService;
...
@Controller
@RequestMapping(value = "/security/hdg/agent_total_stats")
public class AgentTotalStatsController extends SecurityController {

    @Autowired
    AgentTotalStatsService agentDayStatsService;
    @Autowired
    AgentService agentService;

    static Logger log = LoggerFactory.getLogger(AgentTotalStatsController.class);

    @SecurityControl(limits = "hdg.AgentTotalStats:list")
    @RequestMapping(value = "index.htm")
    public void index(Model model) {
        List<Agent> agentList = agentService.findAll();
        model.addAttribute("agentList", agentList);
        model.addAttribute("Category", ConstEnum.Category.values());
        model.addAttribute(MENU_CODE_NAME, "hdg.AgentTotalStats:list");
    }

    @RequestMapping("page.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public PageResult page(AgentTotalStats search) {
    ...
    public ExtResult exportExcel(AgentTotalStats search, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            WritableSheet sheet = wwb.createSheet("运营商总收入统计", 0);
            sheet.getSettings().setDefaultColumnWidth(20);
            sheet.setRowView(0, 400, false);
            /**
             * 设置题头
             */
            sheet.addCell(new Label(0, 0,"运营商总收入统计", getTitleStyle()));
            int column = 0;
            ...
            while (true) {
                ...
                List<AgentTotalStats> list = agentDayStatsService.findForExcel(search);
                if (list.isEmpty()) {
                    break;
                }
                ExportXlsUtils.writeAgentTotalStates(offset, list, sheet);
                offset += limit;
            }
            // 写入数据
            wwb.write();
