...
import cn.com.yusong.yhdg.webserver.service.basic.DeviceUpgradePackService;
...
@Controller
@RequestMapping(value = "/security/basic/device_upgrade_pack")
public class DevicelUpgradePackController extends SecurityController {
    ...
    @SecurityControl(limits = "basic.DeviceUpgradePack:list")
    @RequestMapping("index.htm")
    public void index(String menuCode,Model model) {
        model.addAttribute(MENU_CODE_NAME, menuCode);
        model.addAttribute(MENU_CODE_NAME, "basic.DeviceUpgradePack:list");
    }

    @RequestMapping("page.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public PageResult page(DeviceUpgradePack search) {
        return PageResult.successResult(deviceUpgradePackService.findPage(search));
    }

    @RequestMapping("add.htm")
    @ViewModel(ViewModel.INNER_PAGE)
    public void add(Model model){
        model.addAttribute("typeList", DeviceUpgradePack.DeviceType.values());
    }
    ...
    public String edit(int id, Model model) {
        model.addAttribute("typeList", DeviceUpgradePack.DeviceType.values());
        DeviceUpgradePack upgradePack = deviceUpgradePackService.find(id);
        if(upgradePack == null) {
            return SEGMENT_RECORD_NOT_FOUND;
        }

        model.addAttribute("entity", upgradePack);
        return "/security/basic/device_upgrade_pack/edit";
    }

    @RequestMapping("create.htm")
    @ViewModel(ViewModel.JSON)
    @ResponseBody
    public ExtResult create(DeviceUpgradePack upgradePack) throws Exception{
        if(upgradePack != null){
        ...
        deviceUpgradePackService.insert(upgradePack);
    ...
    public String view(int id, Model model) {
        ...
        model.addAttribute("entity", upgradePack);
        return "/security/basic/device_upgrade_pack/view";
    }

    @ResponseBody
    @ViewModel(ViewModel.JSON)
    @RequestMapping("update.htm")
    public ExtResult update(DeviceUpgradePack upgradePack) throws IOException {
        FileInputStream fileInputStream = null;
        ...
        deviceUpgradePackService.update(upgradePack);
    ...
    public void download(int id, HttpServletRequest request, HttpServletResponse response) throws IOException {
        DeviceUpgradePack entity = deviceUpgradePackService.find(id);
        if(entity == null) {
    ...
    public ExtResult delete(Long id) {
        //如果已有升级版本设备，无法删除
        DeviceUpgradePackDetail packDetail = new DeviceUpgradePackDetail();
        packDetail.setPackId(id.intValue());
        Page page = deviceUpgradePackDetailService.findPage(packDetail);
        List result = page.getResult();
        if (result.size() > 0) {
            return ExtResult.failResult("已有升级版本设备，无法删除");
        } else {
            DeviceUpgradePack deviceUpgradePack = deviceUpgradePackService.find(id);
            File file = appConfig.getFile(deviceUpgradePack.getFilePath());
            if (file.exists()) {
                file.delete();
            }

            return deviceUpgradePackService.delete(id);
        }
    }
