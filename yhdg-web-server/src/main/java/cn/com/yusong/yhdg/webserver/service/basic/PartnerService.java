...
public class PartnerService {
    ...
    public ExtResult create(Partner partner, List<String> sqlList) {
        ...
        insertVoiceMessageTemplateSql(line);

        return ExtResult.successResult(platformAccount.getId()+"");
    }

    // 新建平台时添加语音消息模板数据到对应的平台
    private void insertVoiceMessageTemplateSql(String line) {
        for (String sql : appConfig.voiceMessageTemplateSql) {
            partnerMapper.insertSql(sql.replace("0/*partner_id*/", line));
        }
    }
    ...
    public ExtResult delete(int id) {
        ...
        //已绑定小程序无法删除
        List<Weixinma> byPartnerId = weixinmaMapper.findByPartnerId(id);
        if(byPartnerId.size()>0){
            return ExtResult.failResult("该商户已绑定小程序，无法删除");
        }
        //已经绑定生活号或公众号不能删除
