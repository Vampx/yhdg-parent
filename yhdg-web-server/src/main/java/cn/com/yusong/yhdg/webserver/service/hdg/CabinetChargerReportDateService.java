...
@Service
public class CabinetChargerReportDateService {
    @Autowired
    CabinetChargerReportDateMapper cabinetChargerReportDateMapper;

    public void tree(OutputStream stream, String cabinetId, String boxNum) throws IOException {
        ObjectMapper objectMapper = new ObjectMapper();
        JsonGenerator json = objectMapper.getJsonFactory().createJsonGenerator(stream, JsonEncoding.UTF8);
        json.writeStartArray();
        YhdgUtils.writeJson(buildTree(cabinetId, boxNum), json);
        json.writeEndArray();
    ...
    private List<Node<NodeModel>> buildTree(String cabinetId, String boxNum) {
        List<String> yearList = cabinetChargerReportDateMapper.findYear(cabinetId, boxNum);
        List<Map<String, String>> monthMap = cabinetChargerReportDateMapper.findMonth(cabinetId, boxNum);
        List<CabinetChargerReportDate> list = cabinetChargerReportDateMapper.findList(cabinetId, boxNum);
        ...
        for (String year : yearList) {
            ...
            for (Map<String, String> map : monthMap) {
                ...
                if (map.get("yeah").equals(year)) {
                    ...
                    for (CabinetChargerReportDate date : list) {
