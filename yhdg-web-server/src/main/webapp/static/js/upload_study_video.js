App.uploadStudyVideo = function(config) {
    ...
    var onFileQueued = function(file) {
        var selected_file = getObj('selected_file');
        var html = [
            '<li file_id="FILE_ID" file_name="FILE_NAM' +
            '' +
            '' +
            'E" size="SIZE" size_desc="SIZE_DESC" suffix="SUFFIX">',
    ...
    var onUploadStart = function(file) {
        ...
        var param = {
            clazzId: getObj('clazz_id').combotree('getValue').replace('clazz_',''),
            videoName: encodeURIComponent(App.trimFileName(obj.attr('file_name').replace(/\s/g, '_'), 40)),
            uploadUserId: App.sessionUser.id,
            realName: encodeURIComponent(App.sessionUser.realName),
    ...
    var onFileError = function(file, errorCode) {
        var message = '';
        switch(n){
            case -100 : message = '待上传文件列表数量超限，不能选择！';
                break;
    ...
    function setupSwfUpload() {
        ...
        swfupload = new SWFUpload( {
            ...
            upload_url: App.contextPath + "/security/school/study_video/create.htm",
            flash_url: App.contextPath + "/static/tool/SWFUpload/swfupload.swf",
            file_post_name: "file",
        ...
        getObj('start_upload').click(function() {
            if(!config.schoolId) {
                $.messager.alert('提示消息', '请选择学校', 'info');
                return;
            }
            var t = getObj('clazz_id').combotree('tree');	// 获取树对象
            var n = t.tree('getSelected');		// 获取选择的节点
            if (!n || n.id.indexOf("clazz") == -1) {
                alert('请选择班级');
                return false;
            }
            start();
    ...
    App.dialog.show({
        ...
        title: '上传教学视频',
        href: App.contextPath + "/security/school/study_video/add.htm?schoolId=" + config.schoolId,
        event: {
            onLoad: function(id) {
                pid = $(this).attr('id');
                setupSwfUpload();
