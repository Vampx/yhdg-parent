    ...
    $(function() {
        $(".vip_price_agent_company_add").click(function () {
            App.dialog.show({
                ...
                title: '新建',
                href: "${contextPath}/security/basic/agent_company/vip_rent_price_agent_company_page.htm?agentId=${(agentId)!0}&vipRentFlag=1",
                windowData: {
                    ok: function (rows) {
                        var ids = '';
                        if (rows.length > 0) {
                            ...
                            $.ajax({
                                ...
                                async: false,
                                type: 'POST',
                                url: '${contextPath}/security/zd/vip_rent_price_agent_company/create.htm',
                                dataType: 'json',
                                data: values,
        ...
        $(".vip_price_agent_company .fa-close").click(function () {
            ...
            $.messager.confirm('提示信息', '确认删除该运营公司?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/zd/vip_rent_price_agent_company/delete.htm?id=" + id + "&priceId=" + priceId, function (json) {
                        if (json.success) {
