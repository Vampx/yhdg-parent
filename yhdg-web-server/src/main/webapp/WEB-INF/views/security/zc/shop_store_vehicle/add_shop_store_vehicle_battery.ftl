...
<script>
    ...
    function add_battery() {
        var priceSettingId = $('#price_setting_id').val();
        var batteryCount = $('#battery_count').val();
        var category = $('#category').val();
        var agentId = $('#agent_id').combotree('getValue');
        if (priceSettingId == "" || priceSettingId == null) {
        ...
        if (agentId == "" || agentId == null) {
            ...
            App.dialog.show({
                ...
                title: '新建',
                href: "${contextPath}/security/zc/shop_store_vehicle/shop_store_battery_page.htm?agentId=" + agentId+"&batteryCount="+batteryCount+"&category="+category+"&priceSettingId="+priceSettingId,
                windowData: {
                    ok: function (rows) {
                        var html = "";
