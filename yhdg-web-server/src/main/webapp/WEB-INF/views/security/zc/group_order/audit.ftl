<div class="popup_body" style="min-height: 82%;">
    <div class="ui_table">
        <form method="post">
            ...
            <input type="hidden" name="refundRecordId" <#if refundRecordId ?? >value="${refundRecordId}" </#if>>
            <input type="hidden" name="sourceType" <#if sourceType ?? >value="${sourceType}" </#if>>
            <input type="hidden" name="sourceId" <#if sourceId ?? >value="${sourceId}" </#if>>
        ...
        win.find('button.ok').click(function() {
            $.messager.confirm('提示信息', $('#total_money').html()+'<br/><br/>是否确认?', function (ok) {
                if (ok) {
                    ...
                    $.post('${contextPath}/security/zc/group_order/audit_money.htm', {
                        data: $.toJSON(values)
                    }, function (json) {
                    <@app.json_jump/>
                        if (json.success) {
