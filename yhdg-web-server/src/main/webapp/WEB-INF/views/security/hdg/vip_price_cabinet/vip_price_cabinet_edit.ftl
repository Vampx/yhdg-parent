    ...
    <table id="vip_price_cabinet_list">
        <#if vipPriceCabinetList?? && (vipPriceCabinetList?size>0) >
            <#list vipPriceCabinetList as vipPriceCabinet>
                <tr>
                    <input type="hidden" name="cabinetId" id="cabinet_id" value="${(vipPriceCabinet.cabinetId)!''}">
                    <input type="hidden" name="cabinetName" id="cabinet_name" value="${(vipPriceCabinet.cabinetName)!''}">
                    <td>
                        <input type="text" readonly class="text easyui-validatebox" maxlength="40" style="width:175px;height:28px " value="${(vipPriceCabinet.cabinetName)!''}"
                    </td>
                    <td>
                        <img style="cursor: pointer;" onclick="cabinet_delete(this, ${(vipPriceCabinet.id)!0})" src="${app.imagePath}/delete.png">
                    </td>
                </tr>
            </#list>
        </#if>
...
<script>
    ...
    function cabinet_delete(obj, id) {
            ...
            if(ok) {
                ...
                if (id != null) {
                    ...
                    $.ajax({
                        ...
                        async: false,
                        type: 'POST',
                        url: '${contextPath}/security/hdg/vip_price_cabinet/delete.htm',
                        dataType: 'json',
                        data: values,
                        success: function (json) {
                        <@app.json_jump/>
