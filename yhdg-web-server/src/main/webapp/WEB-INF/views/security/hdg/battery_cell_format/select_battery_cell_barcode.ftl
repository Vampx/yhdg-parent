<div class="popup_body">
    ...
    <div class="select_routes" >
        <div class="select_body" style="margin-right: 20px; top:29px; overflow: hidden;">
            <div class="grid" style="width: 735px; height:360px; margin-right: 20px;">
...
<script>
    ...
    function removeBarcode(id) {
        $.messager.confirm('提示信息', '确认删除?', function(ok) {
            if(ok) {
                $.post('${contextPath}/security/hdg/battery_cell_barcode/delete.htm?id=' + id , function (json) {
                    if (json.success) {
    ...
    function batchRemove() {
        ...
        $.messager.confirm('提示信息', '确认删除?', function (ok) {
            if (ok) {
                $.post('${contextPath}/security/hdg/battery_cell_barcode/batch_remove.htm', {
                    idList: list
                }, function (json) {
                    if (json.success) {
                        $.messager.alert('提示消息', json.message, 'info');
