        ...
        <fieldset style="flex:1">
            <div class="popup_body" style="position: relative;">
                ...
                <div class="zj_item exchange_second" style="position: absolute;right: 0px;top: 8px;background: none">
                    <span class="icon">
                        <i class="fa fa-fw fa-edit" exchange_second_id="${(exchangeSecond.id)!0}" exchange_second_price_id="${(exchangeSecond.packetPriceId)!0}"></i>
                        <i class="fa fa-fw fa-close" exchange_second_id="${(exchangeSecond.id)!0}" exchange_second_price_id="${(exchangeSecond.packetPriceId)!0}"></i>
                    </span>
                </div>
            </div>
        </fieldset>
...
<script>
    ...
    $(function() {
        ...
        $(".exchange_rent_add_second").click(function () {
            ...
            var vipForegiftId = $('#form_first').find('input[name=vipForegiftId]').val();
            var priceId = $('#form_first').find('input[name=priceId]').val();

            var idList = [];
            <#if packetPeriodPriceSecondList??>
                <#list packetPeriodPriceSecondList as packetPeriodPriceSecond>
                    idList.push(${packetPeriodPriceSecond.packetPriceId});
                </#list>
            </#if>
            var id = 0;
            if (idList.length > 0) {
            ...
            $.post('${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_price_create.htm', {
                ...
                if(json.success) {
                    ...
                    App.dialog.show({
                        ...
                        title: '添加租金',
                        href: "${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_price_add_second.htm?batteryType="+ ${(batteryType)!0}+ "&agentId="+ ${(agentId)!0} + "&foregiftId=" + ${(foregiftId)!0} + "&packetPriceId=" + priceId,
                        windowData:{
                        },
                        event: {
                            onClose: function() {
                                $.post('${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_second_price.htm', {
                                    packetPriceId: priceId,
                                    agentId: ${(agentId)!0},
                                    batteryType: ${(batteryType)!0},
                                    foregiftId: ${(foregiftId)!0}
                                }, function (html) {
        ...
        $(".exchange_second .fa-edit").click(function () {
            ...
            App.dialog.show({
                css: 'width:500px;height:425px;overflow:visible;',
                title: '修改租金',
                href: "${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_second_price_edit.htm?exchangeId=" + exchangeId,
                windowData:{
                },
                event: {
                    onClose: function() {
                        $.post('${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_second_price.htm', {
                            packetPriceId: secondPriceId,
                            agentId: ${(agentId)!0},
                            batteryType: ${(batteryType)!0},
                            foregiftId: ${(foregiftId)!0}
                        }, function (html) {
        ...
        $(".exchange_second .fa-close").click(function () {
            ...
            $.messager.confirm('提示信息', '确认删除?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/vip_packet_period_price/exchange_packet_period_second_price_delete.htm?exchangeId=" + exchangeId, function (json) {
                        if (json.success) {
