...
//                        },
                        {
                            ...
                            formatter: function (val, row) {
                                var html = '';
                                <@app.has_oper perm_code='hdg.BatteryCheck:edit'>
                                    html += '<a href="javascript:showBound(\'ID\',\'BARCODE\')" > 修改</a>';
                                </@app.has_oper>
                                <@app.has_oper perm_code='hdg.BatteryCheck:remove'>
                                    html += ' <a href="javascript:remove(\'ID\')">删除</a>';
                                </@app.has_oper>
                                <@app.has_oper perm_code='hdg.BatteryCheck:exchangeQrcode'>
                                    html += ' <a href="javascript:exchangeQrcode(\'ID\')">交换二维码</a>';
                                </@app.has_oper>
                                <@app.has_oper perm_code='hdg.BatteryCheck:bindCode'>
                                    if (row.code == null || row.code == '') {
                                        html += ' <a style="color: #ff0000" href="javascript:updateCode(\'ID\')">绑定BMS编号</a>';
                                    }
                                </@app.has_oper>
                                <@app.has_oper perm_code='hdg.BatteryCheck:editCellBind'>
                                    html += '<a href="javascript:editCellBind(\'ID\',\'BARCODE\')" > 转移电芯</a>';
                                </@app.has_oper>
                                if (row.code) {
                                    <@app.has_oper perm_code='hdg.BatteryCheck:clearCode'>
                                        html += ' <a href="javascript:removeBMS(\'ID\')">清空BMS编号</a>';
                                    </@app.has_oper>
                                }
                                return html.replace(/ID/g, row.id).replace(/BARCODE/g, row.shellCode);
                            }
...
//                shellCode: $('#shell_code').val()
                ...
                success: function (json) {
                    if(json.success) {
                        App.dialog.show({
                            ...
                            title: '修改',
                            href: "${contextPath}/security/hdg/battery_check/show_bound.htm?batteryId=" + batteryId +"&editFlag=1" ,
                            event: {
                                onClose: function () {
                                    reload();
                                }
        ...
        function editCellBind(batteryId) {
            App.dialog.show({
                css: 'width:400px;height:200px;',
                title: '电芯转移',
                href: "${contextPath}/security/hdg/battery_check/edit_cell_bind.htm?batteryId=" + batteryId,
                event: {
                    onClose: function () {
                        reload();
                    }
        ...
        function remove(id) {
            $.messager.confirm('提示信息', '删除该电池会解绑其中的电芯，确认删除?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/battery_check/is_sync_battery.htm?id=" + id, function (json) {
                        if (json.success) {
                            $.messager.confirm('提示信息', '删除该电池会让相应换电电池上的二维码失效，确认删除?', function (ok) {
                                if(ok) {
                                    removeBattery(id);
                                }
                            });
        ...
        function clearCode(id) {
            $.post("${contextPath}/security/hdg/battery_check/clear_code.htm?id=" + id, function (json) {
                if (json.success) {
                    $.messager.alert('提示消息', '操作成功', 'info');
                    reload();
                } else {
        ...
        function removeBMS(id) {
            $.messager.confirm('提示信息', '确认清空BMS编号?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/battery_check/is_sync_battery.htm?id=" + id, function (json) {
                        if (json.success) {
                            $.messager.confirm('提示信息', '清空该电池BMS会让相应换电电池上的二维码失效，确认清空?', function (ok) {
                                if(ok) {
                                    clearCode(id);
                                }
                            });
            ...
            <div class="content">
                        ...
                        <#--</td>-->
                            <td align="right" width="100">
                                <select style="width:100px;" id="query_name">
                                    ...
                                    <option value="shellCode">电池条码编号</option>
                                    <option value="cellBarcode">电芯条码编号</option>
                                </select>
                            </td>
                            <td><input type="text" class="text" style="width: 200px;" id="query_value"/></td>
                        </tr>
                    </table>
                </div>
                <div class="panel grid_wrap" style="margin-top: 30px;">
                    <div class="toolbar clearfix">
                        <div class="float_right">
                        <@app.has_oper perm_code='hdg.BatteryCheck:add'>
