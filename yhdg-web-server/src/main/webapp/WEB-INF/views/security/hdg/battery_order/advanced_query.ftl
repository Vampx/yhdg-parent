        ...
        <form method="post">
            <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                        <select style="width:80px;" id="order_status_${pid}">
...
<script>
    $(function () {
        ...
        $('#order_status_${pid}').val($('#order_status').val());
        $('#pay_type_${pid}').val($('#pay_type').val());
        setTimeout(function () {
            if ($('#agent_id').combotree('getValue') != '') {
                $('#agent_id_${pid}').combotree('setValue', $('#agent_id').combotree('getValue'));
            }
        ...
        win.find('button.ok').click(function () {
            if ($('#agent_id_${pid}').combotree('getValue') != '') {
                $('#agent_id').combotree('setValue', $('#agent_id_${pid}').combotree('getValue'))
            }
            $('#query_value').val($('#query_value_${pid}').val())
            $('#query_name').val($('#query_name_${pid}').val())
            $('#order_status').val($('#order_status_${pid}').val())
