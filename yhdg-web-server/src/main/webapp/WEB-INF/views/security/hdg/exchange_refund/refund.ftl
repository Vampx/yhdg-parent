...
<script>
        ...
        win.find('button.ok').click(function() {
            ...
            $.messager.confirm('提示信息', $('#total_money').html()+'<br/><br/>是否确认?', function (ok) {
                if (ok) {
                    ...
                    $.post('${contextPath}/security/hdg/exchange_refund/refund_money.htm', {
                        data : $.toJSON(values)
                    }, function(json) {
                        <@app.json_jump/>
                        if(json.success) {
    ...
    function checkPayType(){
        ...
        $('#${pid}').find('input[type=checkbox]:checked').each(function () {
            ...
            if(payType==14||payType==15||payType==16){
                return;
            }
        })
        if(payType==14||payType==15||payType==16){
    ...
    $('.changeMoney').click(function () {
        ...
        App.dialog.show({
            ...
            title: '修改金额',
            href: "${contextPath}/security/hdg/exchange_refund/edit_money.htm?refundableMoney=" + refundableMoney +"&realRefundMoney=" + realRefundMoney ,
            windowData: {
