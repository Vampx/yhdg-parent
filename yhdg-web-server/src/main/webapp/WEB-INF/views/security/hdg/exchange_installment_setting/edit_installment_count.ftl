<div class="popup_body" style="padding-left:10px;padding-top: 20px;font-size: 14px;min-height: 69%;">
    <div class="ui_table">
        <form method="post">
            <input type="hidden" id="standard_staging_id" name="standardStagingId" value="${(entity.id)!''}"/>
            <table id="exchange_standard_staging_table">
                <tr>
                    <td width="110" align="left"><span style="color: #990000">*</span>分期期数：</td>
                    <td><input type="text" class="text easyui-validatebox" maxlength="3"
                               style="width:245px;height:28px " id="count_${pid}" data-options="required:true,validType:'integer'"
                               name="count" value="${(entity.count)!''}"/ ></td>
                </tr>
                <tr>
                    <td width="110" align="left"><span style="color: #990000">*</span>手续费类型：</td>
                    <td>
                        <span class="radio_box">
                            <input type="radio" class="radio" name="feeType" <#if entity.feeType??&&entity.feeType==1 >checked</#if>  id="fee_type_1" value="1" />
                                  <label for="fee_type_1">费率</label>
                        </span>
                        <span class="radio_box">
                            <input type="radio" class="radio" name="feeType" id="fee_type_2"<#if entity.feeType??&&entity.feeType==2 >checked</#if>
                                   value="2" /><label for="fee_type_2">固定手续费</label>
                        </span>
                        <span class="radio_box">
                            <input type="radio" class="radio" name="feeType" id="fee_type_3" <#if entity.feeType??&&entity.feeType==3 >checked</#if> value="3" />
                ...
                <tr id="fee_${pid}">
                    ...
                    <td>
                        <input type="text" class="text easyui-validatebox" maxlength="10" style="width:245px;height:28px " id="fee_money_${pid}"
                               name="feeMoney" value="${(entity.feeMoney/100)?string('0.00')}"/>
                        <input type="text" class="text easyui-validatebox" maxlength="4" style="width:245px;height:28px " id="fee_percentage_${pid}"
                               name="feePercentage" value="${(entity.feePercentage/100)?string('0.00')}"/>
                    </td>
                </tr>
            </table>
        </form>
...
<script>
    (function () {
        ...
        win.find('button.ok').click(function () {
            if(verification()) {
                var id = $('#standard_staging_id').val();
                var wins = $('#${urlPid}');
                ...
                if (id != null || id != "" || id != undefined) {
                    $.ajax({
                        ...
                        async: false,
                        type: 'GET',
                        url: '${contextPath}/security/hdg/exchange_installment_count/update_installment_count.htm',
                        data: {
