    ...
    <script>
        $(function() {
            $('#page_table').datagrid({
                ...
                striped: true,
                pagination: true,
                url: "${contextPath}/security/hdg/shop_address_correction/page.htm",
                fitColumns: true,
                pageSize: 50,
                pageList: [50, 100],
                idField: 'id',
                ...
                columns: [
                    [
                        ...
                        },
                        {
                            ...
                            formatter: function(val, row) {
                                var html = '';
                                <@app.has_oper perm_code='4_1_8_2'>
                                    html += '<a href="javascript:view(\'ID\')">查看</a>';
                                </@app.has_oper>
                                if(row.status == 1){
                                        <@app.has_oper perm_code='4_1_8_3'>
                                            html += ' <a href="javascript:pass(\'ID\')">通过</a>';
                                        </@app.has_oper>
                                        <@app.has_oper perm_code='4_1_8_4'>
                                            html += ' <a href="javascript:fail(\'ID\')">不通过</a>';
                                        </@app.has_oper>
                                    return html.replace(/ID/g, row.id);
                                }
                                <@app.has_oper perm_code='4_1_8_5'>
        ...
        function remove(id) {
            $.messager.confirm('提示信息', '确认删除?', function(ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/shop_address_correction/delete.htm?id=" + id, function (json) {
                        if (json.success) {
        ...
        function pass(id) {
            $.messager.confirm('提示信息', '确认通过?', function(ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/shop_address_correction/update.htm?id=" + id + "&status=2", function (json) {
                        if (json.success) {
                            $.messager.alert('提示消息', '操作成功', 'info');
                            reload();
                        } else {
        ...
        function fail(id) {
            $.messager.confirm('提示信息', '确认不通过?', function(ok) {
                if (ok) {
                    $.post("${contextPath}/security/hdg/shop_address_correction/update.htm?id=" + id + "&status=3", function (json) {
                        if (json.success) {
                            $.messager.alert('提示消息', '操作成功', 'info');
                            reload();
                        } else {
        ...
        function view(id) {
            App.dialog.show({
                ...
                title: '查看',
                href: "${contextPath}/security/hdg/shop_address_correction/view.htm?id=" + id
            });
        }
