...
<script>
    ...
    (function() {
        ...
        win.find('button.ok').click(function() {
            ...
            if (Math.round(reduceMoney * 100) > Math.round(money * 100)) {
                $.messager.alert('提示信息', "减免金额应小于押金金额", 'info');
                return false;
            }

            form.form('submit', {
              url: '${contextPath}/security/hdg/vip_exchange_battery_foregift/vip_foregift_create.htm',
                onSubmit: function(param) {
                    param.foregiftId = windowData.foregiftId;
                    param.agentId = windowData.agentId;
                    param.batteryType = windowData.batteryType;
                    param.reduceMoney = parseInt(Math.round(reduceMoney * 100));
                    param.money = parseInt(Math.round(money * 100));
                    param.num = windowData.index;
                    return true;
                },
                ...
                <@app.json_jump/>
                    if(json.success) {
                        ...
                        if (win.find('input[name=id]').val() == 0) {
                            windowData.ok({
                                vipExchangeId: json.data,
                                memo: win.find('textarea[name=memo]').val(),
                                reduceMoney: parseInt(Math.round(reduceMoney * 100))
                            });
                        } else {
                            windowData.ok({
                                vipExchangeId: win.find('input[name=id]').val(),
                                memo: win.find('textarea[name=memo]').val(),
                                reduceMoney: parseInt(Math.round(reduceMoney * 100))
                            });
                        }
