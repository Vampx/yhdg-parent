...
<script>
    ...
    $(function() {
        ...
        $(".exchange_rent_add_first").click(function () {
            ...
            var vipForegiftId = $(this).parents().find(".vip_battery_foregift_list").find(".active").find(".edit_foregift").attr("vip_exchange_id");
            ...
            if (vipForegiftId == 0) {
                ...
                App.dialog.show({
                    ...
                    title: '添加租金',
                    href: "${contextPath}/security/hdg/vip_packet_period_price/vip_exchange_packet_period_price_add_first.htm?batteryType="+ batteryType+ "&agentId="+ agentId + "&foregiftId=" + foregiftId+ "&vipForegiftId=" + vipForegiftId,
                    windowData:{
                        ok: function (config) {
                            if (config) {
