<@app.html>
    ...
    <script>
                            ...
                            formatter: function (val, row) {
                                ...
                                <@app.has_oper perm_code='basic.Withdraw:audit'>
                                    if (row.status == 1) {
                                        ...
                                        html += '<a href="javascript:offline(\'ID\')" style="margin-left: 10px;">线下提现</a>';
                                    }
                                </@app.has_oper>
        ...
        function query() {
            ...
            var belongAgentId = $('#belong_agent_id').combotree('getValue');
        ...
        function offline(id) {
            $.messager.confirm('提示信息', '确认线下提现?', function (ok) {
                if (ok) {
                    $.ajax({
                        cache: false,
                        async: false,
                        type: 'POST',
                        url: '${contextPath}/security/basic/withdraw/offline.htm?id=' + id,
                        dataType: 'json',
                        success: function (json) {
                            <@app.json_jump/>
                            if (json.success) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <select class="easyui-combobox" name="type" style="width: 182px; height: 30px;">
                                        ...
                                        <option value="4">平台账户</option>
                                    </select>
                                </td>
                                <td width="60" align="right">运营商：</td>
                                <td>
                                        <input name="belongAgentId" id="belong_agent_id" class="easyui-combotree" editable="true"
