<div class="popup_body">
    <div class="ui_table">
        <form method="post">
            ...
            <input type="hidden" name="fromAlipayfwId" value="${(entity.alipayfwId)!''}">
            <table cellpadding="0" cellspacing="0">
                <#--<tr>-->
                    ...
                    <#--<#list agentList as s>-->
                        <#--<option value="${s.getId()}" <#if entity.alipayfwId?? && entity.alipayfwId == s.getId()>selected</#if> >${s.getAgentName()}</option>-->
                    <#--</#list>-->
                    <#--</select></td>-->
                <#--</tr>-->
                <tr>
...
<script>
    (function() {
        ...
        var snapshot = $.toJSON({
            id: '${(entity.id)!''}',
            <#--alipayfwId: '${(entity.alipayfwId)!''}',-->
            templateName: "${(entity.templateName)!''}",
            variable: '${(entity.variable?js_string)!''}',
            fwCode: '${(entity.fwCode)!''}',
        ...
        var ok = function() {
            ...
            var values = {
                id: '${(entity.id)!''}',
                <#--alipayfwId: $('#app_id_${pid}').combobox('getValue'),-->
                templateName: form.templateName.value,
                variable: form.variable.value,
                fwCode: form.fwCode.value,
            ...
            if(snapshot != $.toJSON(values)) {
                $.ajax({
                    ...
                    async: false,
                    type: 'POST',
                    url: '${contextPath}/security/basic/fw_push_message_template/update_basic.htm',
                    dataType: 'json',
                    data: {
                        id: '${(entity.id)!''}',
                        <#--fromAppId: '${(entity.alipayfwId)!''}',-->
