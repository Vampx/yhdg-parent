...
<div class="select_city" id="area_selector_${selectorId}" style="position:absolute; top: 200px; z-index:9999; display:none;">
    <input type="hidden" id="area_text_${selectorId}" class="text" readonly
           value="${(areaText)!''}"
    ...
    />
    <input type="hidden" id="province_id_${selectorId}" name="provinceId" value="${(provinceId)!''}">
    <input type="hidden" id="city_id_${selectorId}" name="cityId" value="${(cityId)!''}">
    <input type="hidden" id="district_id_${selectorId}" name="districtId" value="${(districtId)!''}">
    <div class="dialog city_box" style="top:0px; left:0; display:block;">
        <div class="dialog_title">
            <h3>请选择地区</h3>
            <a class="close" href="javascript:void(0)"><span class="icon"></span></a>
            <a class="ok" href="javascript:void(0)"><span class="icon"></span></a>
        </div>
            ...
            <script>
                ...
                $(function(){
                    ...
                    function close_selector() {
                        $("#mask_${selectorId}").remove();
                        $("#area_selector_${selectorId}").remove();
                    }
                    ...
                    $("#area_selector_${selectorId} .tab_con .tab_item:eq(3) .city_list").delegate('a', 'click', function() {
                        ...
                        var value = {
                            areaText: $('#area_text_${selectorId}').val(),
                            provinceId: $('#province_id_${selectorId}').val(),
                            cityId: $('#city_id_${selectorId}').val(),
                            districtId: $('#district_id_${selectorId}').val()
                        };
                        $('#area_selector_${selectorId}').data('fn')(value);
                    ...
                    <dl class="city_list clearfix">
                        ...
                        <#list controller.appConfig.areaCache.rootList as area>
                            <#if area.letter == "a" || area.letter == "b" || area.letter == "c" || area.letter == "d" || area.letter == "e" || area.letter == "f" || area.letter == "g">
                                <a href="javascript:void(0)" area_id="${area.id}" area_level="${area.areaLevel}">${area.areaName}</a>
                            </#if>
                        </#list>
                        </dd>
                    </dl>
