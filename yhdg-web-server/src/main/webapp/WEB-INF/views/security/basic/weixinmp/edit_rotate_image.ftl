...
<script>
    (function() {
        ...
        $("#page_table_${pid}").datagrid({
            ...
            striped: true,
            pagination: true,
            url: "${contextPath}/security/basic/weixinmp/page_rotate_image.htm",
    ...
    function add_rotate_image() {
        App.dialog.show({
            ...
            title: '新建',
            href: "${contextPath}/security/basic/weixinmp/add_rotate_image.htm?sourceId="+$('#sourceId').val(),
            event: {
                onClose: function() {
                    reload();
                }
    ...
    function edit_rotate_image_edit(id) {
        App.dialog.show({
            css: 'width:380px;height:300px;',
            title: '修改',
            href: "${contextPath}/security/basic/weixinmp/edit_rotate_image_edit.htm?id=" + id,
            event: {
                onClose: function() {
                    reload();
                },
    ...
    function view_rotate_image_view(id) {
        App.dialog.show({
            css: 'width:380px;height:300px;',
            title: '查看',
            href: "${contextPath}/security/basic/weixinmp/view_rotate_image_view.htm?id=" + id,
            event: {
                onClose: function() {
                    reload();
                },
    ...
    function remove_rotate_image_remove(id) {
        $.messager.confirm('提示信息', '确认删除?', function(ok) {
            if(ok) {
                $.post('${contextPath}/security/basic/weixinmp/remove_rotate_image_remove.htm', {
                    id: id
                }, function (json) {
                    if (json.success) {
