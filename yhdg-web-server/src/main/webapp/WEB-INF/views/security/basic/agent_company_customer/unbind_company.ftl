            ...
            <div class="select_body" style="top:59px; overflow: hidden;">
                ...
                <#if customerList ??>
                    <#list customerList as customer>
                        <li customerId="${customer.id}" customerFullname ="${(customer.fullname)!''}"><span class="text" >${(customer.fullname)!''}</span></li>
                    </#list>
                </#if>
                </ul>
            </div>
...
<script>
    ...
    (function() {
        ...
        datagrid.datagrid({
            ...
            striped: true,
            pagination: true,
            url: "${contextPath}/security/basic/agent_company_customer/unbind_company_page.htm?agentId=${(agentId)!''}&agentCompanyId=${(agentCompanyId)!''}&unbindCompanyFlag=1",
            fitColumns: true,
            pageSize: 10,
            pageList: [10, 50, 100],
            idField: 'id',
            ...
            columns: [
                [
                    ...
                    {title: '骑手姓名', align: 'center', field: 'fullname', width: 60},
                    {title: '骑手手机号', align: 'center', field: 'mobile', width: 60}
                ]
            ],
        ...
        var line =
                '<li customerId="CUSTOMER_ID" customerFullname="CUSTOMER_FULLNAME">' +
                '<span class="text">CUSTOMER_ID/CUSTOMER_FULLNAME</span>' +
        ...
        var add = function(customer) {
            if($('#selected_container_${pid} li[customerId="' + customer.id +'"]').length > 0) {
                alert("骑手已存在");
                return;
            }
            var name;
            ...
            var html = line.replace(/CUSTOMER_ID/g, customer.id).replace(/CUSTOMER_FULLNAME/g, name);
