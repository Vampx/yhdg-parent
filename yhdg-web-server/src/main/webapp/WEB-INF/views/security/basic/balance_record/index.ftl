    ...
    <script>
        $(function () {
            $('#page_table').datagrid({
                ...
                collapsible: true,
                pagination: true,
                url: "${contextPath}/security/basic/balance_record/page.htm",
                pageSize: 50,
                pageList: [50, 100],
                idField: 'id',
                frozenColumns: [[
...
//                                html += '<a href="javascript:view(ID)">查看明细</a>';
                            <#--</@app.has_oper>-->
                                if(row.status == 3) {
                                    html += '<a href="javascript:view_confirm_record(ID)">确认记录</a>';
                                }
        ...
        function view_confirm_record(id) {
            App.dialog.show({
                css: 'width:430px;height:427px;overflow:visible;',
                title: '查看',
                href: "${contextPath}/security/basic/balance_record/view_confirm_record.htm?id=" + id
            });
        }
        ...
        function edit_status_offline() {
            ...
            if (ids.length == 0) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    if (rows[i].status != 1) {
                        $.messager.alert('提示消息', "请选状态为待确认的数据！", 'info');
                        return;
                    }
                }
                var totalMoney = 0;
                for (var i = 0; i < rows.length; i++) {
                    totalMoney = totalMoney + rows[i].money;
                }
                App.dialog.show({
                    css: 'width:500px;height:340px;overflow:visible;',
                    title: '确认(线下)',
                    href: "${contextPath}/security/basic/balance_record/edit_status_offline.htm?ids=" + ids + "&totalMoney=" + totalMoney,
                    event: {
                        onClose: function () {
                            reload();
                        }
        ...
        function view_province_income(id) {
            App.dialog.show({
                css: 'width:330px;height:155px;overflow:visible;',
                title: '省代收入',
        ...
        function view_foregift_remain_money(id) {
            App.dialog.show({
                css: 'width:1050px;height:510px;overflow:visible;',
                title: '押金剩余金额',
                href: "${contextPath}/security/basic/balance_record/view_foregift_remain_money.htm?id=" + id
            });
        }

        function view_refund_insurance_money(id) {
            App.dialog.show({
                css: 'width:1050px;height:510px;overflow:visible;',
                title: '保险退款',
                href: "${contextPath}/security/basic/balance_record/view_refund_insurance_money.htm?id=" + id
            });
        }

        function view_refund_packet_period_money(id) {
            App.dialog.show({
                css: 'width:1050px;height:510px;overflow:visible;',
                title: '包时段退款',
                href: "${contextPath}/security/basic/balance_record/view_refund_packet_period_money.htm?id=" + id
            });
        }

        function view_deduction_ticket_money(id) {
            App.dialog.show({
                css: 'width:1050px;height:510px;overflow:visible;',
                title: '抵扣券',
                href: "${contextPath}/security/basic/balance_record/view_deduction_ticket_money.htm?id=" + id
            });
        }
            ...
            <div class="content">
                <div class="panel search">
                    ...
                    <table cellpadding="0" cellspacing="0" border="0">
                        <tr>
                            <#if Session['SESSION_KEY_USER'].agentId == 0>
                                ...
                                <td>
                                    <input type="text" name="agentId" id="agent_id" class="easyui-combotree"
                                           editable="true" style=" height: 28px;width: 200px"
                            ...
                            <td>
                                <input type="text" class="text" id="shopName" style="width: 90px;height: 28px;"/>&nbsp;&nbsp;&nbsp;&nbsp;
                            </td>
                ...
                <div class="panel grid_wrap">
                    <div class="toolbar clearfix">
                        <div class="float_right">
                        <@app.has_oper perm_code='basic.BalanceRecord:confirm'>
                            <button class="btn btn_green" onclick="confirm_status()">确认</button>
                        </@app.has_oper>
