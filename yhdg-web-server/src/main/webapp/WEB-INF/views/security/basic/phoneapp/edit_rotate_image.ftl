...
<script>
    ...
    function add_rotate_image() {
        App.dialog.show({
            ...
            title: '新建',
            href: "${contextPath}/security/basic/phoneapp/add_rotate_image.htm?sourceId="+$('#sourceId').val(),
            event: {
                onClose: function() {
                    reload();
                }
    ...
    function edit_rotate_image_edit(id) {
        App.dialog.show({
            css: 'width:380px;height:300px;',
            title: '修改',
            href: "${contextPath}/security/basic/phoneapp/edit_rotate_image_edit.htm?id=" + id,
            event: {
                onClose: function() {
                    reload()
                },
    ...
    function remove_rotate_image_remove(id) {
        $.messager.confirm('提示信息', '确认删除?', function(ok) {
            if(ok) {
                $.post('${contextPath}/security/basic/phoneapp/remove_rotate_image_remove.htm', {
                    id: id
                }, function (json) {
                    if (json.success) {
