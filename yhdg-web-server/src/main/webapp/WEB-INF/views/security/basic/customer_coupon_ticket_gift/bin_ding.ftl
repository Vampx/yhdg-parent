<script>
        (function () {
            ...
            datagrid.datagrid({
                ...
                striped: true,
                pagination: true,
                url: "${contextPath}/security/basic/customer_coupon_ticket_gift/bin_ding_page.htm",
                fitColumns: true,
                pageSize: 10,
                pageList: [10, 50, 100],
                idField: 'id',
        ...
        function untYing(id) {
            $.messager.confirm('提示信息', '确认解绑?', function (ok) {
                if (ok) {
                    $.post("${contextPath}/security/basic/customer_coupon_ticket_gift/untYing.htm?id=" + id, function (json) {
                        if (json.success) {
        ...
        function add_riders_${pid}() {
            App.dialog.show({
                ...
                title: '添加骑手手机号',
                href: "${contextPath}/security/basic/customer_coupon_ticket_gift/add_rider.htm?ticketGiftId=" + '${ticketGiftId!}',
                event: {
                    onClose: function () {
                        reload();
                    },
