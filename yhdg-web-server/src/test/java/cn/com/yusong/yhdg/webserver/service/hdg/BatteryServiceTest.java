...
public class BatteryServiceTest extends BaseJunit4Test {
    ...
    public void pageUpgrade() {
        ...
        assertTrue(1 == service.pageUpgrade(battery).getTotalItems());
        assertTrue(1 == service.pageUpgrade(battery).getResult().size());
    }
    ...
    public void findCountByPositionState() {
        ...
        assertTrue(1 == service.findCountByPositionState(battery.getPositionState(), battery.getIsOnline(), battery.getAgentId()));
    ...
    public void findList() {
        ...
        insertAgent(agent);
          ...
          SystemBatteryType systemBatteryType = newSystemBatteryType();        insertSystemBatteryType(systemBatteryType);        Battery battery = newBattery(agent.getId(), systemBatteryType.getId());        insertBattery(battery);
    ...
    public void checkBindParam() {
        ...
        DataResult dataResult = (DataResult) service.checkBindParam(battery.getCode(), battery.getShellCode());
        assertTrue(dataResult.isSuccess());
        Battery battery1 = (Battery) dataResult.getData();
        assertEquals(battery.getId(), battery1.getId());
    }

    @Test
    public void checkBindParam2() {
        Partner partner = newPartner();
        insertPartner(partner);
    ...
    public void createCheckedBattery() {
        ...
        DataResult dataResult = (DataResult) service.createCheckedBattery(battery);
        assertTrue(dataResult.isSuccess());
        Battery battery1 = (Battery) dataResult.getData();
        assertEquals(battery1.getId(), battery.getId());
    }
    ...
    public void update(){
        ...
        Battery battery = newBattery(agent.getId(), systemBatteryType.getId());
        battery.setUpLineStatus(BatteryInstallRecord.Status.YESONLINE.getValue());
        insertBattery(battery);
...
//      assertEquals(95, jdbcTemplate.queryForMap("select charge_complete_volume from hdg_battery where id = ?", battery1.getId()).get("full_volume"));
