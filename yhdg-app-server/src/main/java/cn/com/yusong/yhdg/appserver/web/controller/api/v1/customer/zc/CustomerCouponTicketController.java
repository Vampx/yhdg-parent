...
@Controller("api_v1_customer_zc_customer_coupon_ticket")
@RequestMapping(value = "/api/v1/customer/zc/customer_coupon_ticket")
public class CustomerCouponTicketController extends ApiController {
    ...
    public RestResult list(@Valid @RequestBody ListParam param) {
        ...
        List<CustomerCouponTicket> ticketList = customerCouponTicketService.findList(param.agentId,
                customer.getMobile(), new Date(), null, CustomerCouponTicket.Status.NOT_USER.getValue(), CustomerCouponTicket.Category.VEHICLE.getValue(),null, null);
        if (ticketList != null) {
            for (CustomerCouponTicket customerCouponTicket : ticketList) {
                ...
                mapTicket.put("ticketType", customerCouponTicket.getTicketType());
