...
@Controller("api_v1_cabinet_app_basic_customer_coupon_ticket")
@RequestMapping(value = "/api/v1/cabinet_app/basic/customer_coupon_ticket")
public class CustomerCouponTicketController extends ApiController {
    ...
    public RestResult list(@Valid @RequestBody ListParam param) {
        ...
        List<CustomerCouponTicket> ticketList = customerCouponTicketService.findList(customer.getAgentId(), customer.getMobile(),null, param.type, CustomerCouponTicket.Status.NOT_USER.getValue(), CustomerCouponTicket.Category.EXCHANGE.getValue(), null, null);
        if (ticketList != null) {
            for (CustomerCouponTicket customerCouponTicket : ticketList) {
                ...
                mapTicket.put("ticketName", customerCouponTicket.getTicketName() == null ? "" : customerCouponTicket.getTicketName());
                mapTicket.put("money", customerCouponTicket.getMoney() == null ? 0 : customerCouponTicket.getMoney());
                mapTicket.put("expireTime", customerCouponTicket.getExpireTime() != null ?DateFormatUtils.format(customerCouponTicket.getExpireTime(), Constant.DATE_FORMAT) : "");
                list.add(mapTicket);
            }
        }
