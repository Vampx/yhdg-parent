...
@Controller("api_v1_cabinet_app_basic_zhizu_customer")
@RequestMapping(value = "/api/v1/cabinet_app/basic/zhizu_customer")
public class ZhizuCustomerController extends ApiController {
    final static Logger log = LogManager.getLogger(ZhizuCustomerController.class);
    ...
    public RestResult listByCabinet(@Valid @RequestBody ListParam param) {
        ...
        if(param.syncType == 1){ //全量
            List<ZhizuCustomer> customerList =  zhizuCustomerService.findAllList(now);
            for(ZhizuCustomer customer : customerList){
                Map map = new HashMap();
                map.put("id", customer.getId());
                map.put("mobile", customer.getRealMobile());
                map.put("idCard", customer.getRealIdCard());
                map.put("rentTime", customer.getRentTime());
                map.put("batteryType", customer.getBatteryType());
                list.add(map);
            }
        }else if(param.syncType == 2){ //增量
            List<ZhizuCustomer> customerList =  zhizuCustomerService.findIncrementList(new Date(param.syncTime));
            for(ZhizuCustomer customer : customerList){
                Map map = new HashMap();
                map.put("id", customer.getId());
                map.put("mobile", customer.getRealMobile());
            ...
            return RestResult.dataResult(RespCode.CODE_2.getValue(), "类型不符合", null);
        }

        Map data = new HashMap();
        data.put("syncTime", now.getTime());
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    public static class SyncSuccessParam {
        public int syncType;
        public long syncTime;
        public int count;
    }
    ...
    @ResponseBody
    @RequestMapping(value = "/sync_success.htm")
    public RestResult SyncSuccess(@Valid @RequestBody SyncSuccessParam param) {
        TokenCache.Data tokenData = getTokenData();
        String cabinetId = tokenData.cabinetId;

        Date syncTime = new Date(param.syncTime);

        Integer count = null;
        if(param.syncType == 1){ //全量
            count =  zhizuCustomerService.findAllCount();
        }else if(param.syncType == 2){ //增量
            count =  zhizuCustomerService.findIncrementCount(syncTime);
        }else{
            return RestResult.dataResult(RespCode.CODE_2.getValue(), "类型不符合", null);
        }

        if(count != null && count == param.count){
            //取消通知
            zhizuCustomerNoticeService.delete(cabinetId);
            return RestResult.dataResult(RespCode.CODE_0.getValue(), null, null);
        }else{
            return RestResult.dataResult(RespCode.CODE_2.getValue(), "同步条数不对", null);
        }
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    public static class NextParam {
        public String customerMobile;
    }

    @ResponseBody
    @RequestMapping(value = "/next")
    public RestResult next(@Valid @RequestBody NextParam param) throws ExecutionException, InterruptedException {
        TokenCache.Data tokenData = getTokenData();
        String cabinetId = tokenData.cabinetId;
        ...
        return handleCabinetQrcode(customer, cabinet);


    }

    private RestResult handleCabinetQrcode(Customer customer, Cabinet cabinet) throws ExecutionException, InterruptedException {
        ...
        if(batteryList.size() > 0){
            ...
            if(backBatteryOrder != null){
                ...
                return RestResult.dataResult(RespCode.CODE_0.getValue(), "您已申请退押金，欢迎再次使用", data);
            }
    ...
    private RestResult batteryOrderOpenEmptyBox(String cabinetId, Long customerId, Integer batteryType) throws InterruptedException {
        ...
        int openSuccess = 0;
        Map data = new HashMap();
        data.put("cabinetId", subcabinetId);
        ...
        if (openSuccess == 1) {
            insertCabinetOperateLog(cabinet.getAgentId(),
                    ...
                    CabinetOperateLog.OperateType.OPEN_DOOR,
                    CabinetOperateLog.OperatorType.CUSTOMER,
                    String.format("客户%s %s, 换电订单打开空箱成功（屏幕）", customer.getFullname(), customer.getMobile()),
                    customer.getFullname());
